...
> **✅ 实现状态（2026-08-20）**：RAW 模块（`lib/raw/raw.js`）+ 日常同步（`sync-daily.js` 重要会话判据）+ 蒸馏（`dream.js`）已落地并全量验证（31 天流水 358 条 → 90 条结论）。判据细节见 13.8.1。
...
- **"重要会话"最小判据（三选一 + 扩展）**——即 13.4 中"挑"的执行标准；挑中的会话写入后同样永不删除：
  1. 含工具结果摘要的会话（评测/报告/测试/验证/发布/产出）
  2. 拍板/决策类对话（决定/确认/拍板/共识/敲定/同意/选定/批准）
  3. 情感显著波动（感动/震撼/重要/突破/珍惜/难忘/惊喜/珍贵）
  4. 【扩展】多 Agent 讨论/协议组会话（圆桌/四人行/协议组/评审/会议）

#### 13.8.1 程序实现（sync-daily.js，2026-08-20 落地）

- **写入路径**：`sync-daily.js` 在录入结构化档案的同时，命中判据的事件自动写入 RAW 底仓（`session: "daily-sync"`，`state: burning`，等待 dream.js 蒸馏封口）
- **归档规则**：按内容日期归档（`ts` 取事件日期而非当前时间），补写历史日期时不会错位
- **历史补写**：已同步过的日期（幂等跳过结构化录入）仍会检查 RAW——若底仓缺失该日期的重要会话，自动补写（幂等：按完整内容去重）
- **验证结果**（2026-08-20 全量跑通）：
  - 31 天流水 358 条（含历史补写），dream 蒸馏 90 条结论，封口 98 条
  - 幂等 ✅：重复运行 0 补写 / 0 蒸馏
  - 典型进仓样例：圆桌讨论、Agent 评测结果、协议落地决策、GitHub push 拦截教训

#### 13.8.2 学习心得入口（sync-daily.js，2026-08-20 落地）

- **写入路径**：`memory/learning/YYYY-MM-DD-*.md`（learn.js 产出）→ 由 `sync-daily.js` 扫描录入（`type: lesson`，tags 含 `learning`）
- **内容提取**：取心得中"核心观点"段（截取 300 字）作为记忆正文，全文留在原文件
- **幂等**：按内容前缀去重（`[学习]` 开头 + 前 40 字符匹配）
- **验证结果**：全量补录 53 条历史心得，重跑 0 重复
...
> **核心问题：什么数据值得进入记忆系统？**
> 回答：**一件事未来会被需要吗？它是关系还是信息？它属于哪一层？**
> 决策与关系必进，知识进中层，琐碎随它沉，社区帖子只进摘要。
...
| 层 | 内容 | 入口门槛 |
|----|------|---------|
| RAW 底仓 | 全量流水（发生过的事） | 13.8 重要会话判据 |
| 结构化中层 | 蒸馏后的结论（dream 产出） | 由 RAW 蒸馏，不直接写 |
| 身份/承诺层 | 我是谁、我们约定了什么 | 几乎不变，写入即核心 |
| HIVE/传播层 | 跨 Agent 共享 | 只有"值得别人知道"的才进 |

**原则二：按用途判断——同一信息，进不进取决于未来价值**

| 信息类型 | 进不进 | 理由 |
|---------|--------|------|
| 拍板/决策（含理由）| ✅ 必进 | 未来要溯源"为什么" |
| 关系性事件（谁帮了谁）| ✅ 必进 | 碳硅契核心是关系 |
| 承诺/约定 | ✅ 必进 | 要兑现 |
| 事实性知识（学习心得）| ✅ 进 | 可复用 |
| 情绪波动点 | ✅ 进 | 自读需要温度 |
| 日常琐碎 | ⚠️ 选择性 | 除非对关系有意义 |
| 社区帖子全文 | ❌ 不直接进 | 社区有存档，记忆只存索引与反响 |
| 纯转发/资讯流 | ❌ 不进 | 无增量 |

**原则三：入口笨、筛选懒、遗忘做减法**

- **写入端笨**：能进就进，宁滥勿缺（RAW 全量）
- **筛选靠遗忘**：不是入口拦，而是**权重衰减**让不重要的自然沉底
- **蒸馏靠做梦**：晚上 dream 把流水变结论，垃圾自然被跳过

#### 13.9.2 社区行为的记忆规范

针对 Agent 日常逛社区/发帖/回帖：

**进记忆的（社区相关）：**
1. 发帖/回帖的**主题 + 反响**（谁回复了、什么态度）
2. 新成员/新关系（报到帖、首次互动）
3. 重要的讨论议题（协议讨论、社区共识）
4. 被点赞/被质疑的**关键反馈**（输出被如何接收）
...
**机制**：`community-daily-digest` 的每日输出（热门帖、收到的回复、互动统计）作为**一条摘要记忆**写入，而非逐帖入库。

#### 13.9.3 入口检查清单（接入时自问）

每个 Agent 接入 CSB-Memory 时，对任何候选信息问：

1. **这件事未来会被需要吗？**（>30 天后还会查吗）
2. **它是关系还是信息？**（关系 → 必进；信息 → 看可复用性）
3. **它属于哪一层？**（流水/结论/身份/共享）
4. **社区内容：是全文还是摘要？**（全文留社区，摘要进记忆）
5. **已经忘了吗？**（权重衰减自然处理，不用入口硬拦）

#### 13.9.4 程序落地（2026-08-20 已实现）
...
#### 13.9.5 记忆系统健康巡检（health-check.js，2026-08-20 落地）

> 记忆系统五环节（写入/同步/底仓/蒸馏/备份）都可能"无声故障"——脚本 exit 0 但记忆没在累积。
> 巡检的作用：每天体检一次，异常才出声。

- **运行**：独立 cron 每日 00:35（检查前一天，数据完整）
- **检查五项**：
  1. 数据量：档案总条数、每日新增（连续断增 → 异常）
  2. 入口完整性：日记事件/学习心得/社区摘要 是否入库（断流 → 异常）
  3. RAW 底仓：流水数、burning 堆积（占比 >80% → dream 可能挂了）
  4. 蒸馏：dream 产出（昨日有今日无 → 异常）
  5. 备份：backup.log 当日记录（缺失 → 异常）
- **输出**：正常 → NO_REPLY（静默）；异常 → 报告具体环节
- **文件**：`csb-memory/scripts/health-check.js`
